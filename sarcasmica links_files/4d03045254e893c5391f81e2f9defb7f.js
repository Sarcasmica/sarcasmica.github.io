...
highlightPopularsBy(needle,isInputEmpty){needle=this.stripProtocol(needle);this.opts.$popularLinkContainers.show().filter(function(){return $(this).data('hostname').indexOf(needle)===-1}).hide();}
highlightPaymentsBy(needle,isInputEmpty){needle=this.stripProtocol(needle);this.opts.$paymentsLinkContainers.show().filter(function(){return $(this).data('hostname').indexOf(needle)===-1}).hide();}
...
stripProtocol(str){return str.replace(/^(http|https):\/\//,'');}};