...
changeUsername(){let usernameVal=this.opts.$usernameInput.val();if(usernameVal.includes('https://')||usernameVal.includes('http://')){usernameVal=(new URL(usernameVal)).pathname.slice(1);}
if(usernameVal.indexOf(this.opts.hostname+'/')===0){usernameVal=usernameVal.slice((this.opts.hostname+'/').length);}
let newUrl=this.opts.urlTemplate.replace('{hostname}',this.opts.hostname).replace('{username}',usernameVal);let urlTitle=newUrl.replace(/^(http|https):\/\//,'');this.opts.$descr_link.attr('href',newUrl).text(urlTitle);this.opts.$contentInput.val(newUrl);}
